...
router.route('/get').get((req, res) => {
    ...
    if(req.query.q){
        ...
        saleModel.find({$or:[{ 'customer': new RegExp(q, 'i') }, { 'product': new RegExp(q, 'i') }]}, (err, docs) => {
            if (!err) {
                res.send(docs);
            } else {
        ...
        saleModel.find({}).sort({name: 1}).exec((err, docs) => {
            if (!err) {
                res.send(docs);
            } else {
                console.log('Failed to retrieve the Course List: ' + err);
...
router.route('/delete').post((req, res) => {
    var myquery = { _id: new ObjectId(req.body.id) };
    db.collection("sales").deleteOne(myquery, function(err, res) {
        if (err) throw err;
    });
    res.send("Deleted");
});
